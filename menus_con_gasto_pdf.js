...
/* Calcula el gasto aproximado de una porción usando su peso y el precio por cada 100 gramos. */
function calcularGastoAlimento(item){
    const peso = Number(item.pesoBruto) || 0;
    const precioPor100g = Number(item.precio100g) || 0;
    return (peso * precioPor100g) / 100;
}

/* Suma el gasto aproximado de todos los alimentos agregados por el usuario. */
function calcularGastoTotalMenu(){
    return menuUsuario.reduce(function(total, item){
        return total + calcularGastoAlimento(item);
    }, 0);
}

/* Muestra importes monetarios con dos decimales y formato de pesos mexicanos. */
function formatearMoneda(valor){
    const numero = Number(valor);
    return "$" + (Number.isFinite(numero) ? numero.toFixed(2) : "0.00") + " MXN";
}
...
function obtenerResumenCategorias(){ // Inicia la función obtenerResumenCategorias para realizar esta tarea cuando sea llamada.
    ...
    menuUsuario.forEach(function(item){ // Recorre cada elemento de la lista y ejecuta la función para cada uno.
        ...
        if(!resumen[categoria]){ // Comprueba esta condición antes de ejecutar el bloque siguiente.
            resumen[categoria] = { categoria: categoria, cantidad: 0, kcal: 0, gasto: 0 }; // Ejecuta esta instrucción como parte de la lógica de la función actual.
        } // Cierra el bloque de código anterior.

        resumen[categoria].cantidad++; // Ejecuta esta instrucción como parte de la lógica de la función actual.
        resumen[categoria].kcal += Number(item.kcal) || 0;
        resumen[categoria].gasto += calcularGastoAlimento(item); // Ejecuta esta instrucción como parte de la lógica de la función actual.
    }); // Cierra la estructura, función o llamada que se abrió en líneas anteriores.
...
function construirReportePDF(doc){ // Inicia la función construirReportePDF para realizar esta tarea cuando sea llamada.
    ...
    const gastoTotal = calcularGastoTotalMenu(); // Suma el costo aproximado de las porciones seleccionadas.
    const gastoPromedio = menuUsuario.length ? gastoTotal / menuUsuario.length : 0; // Calcula el costo promedio por alimento agregado.
    ...
    tituloSeccion("Gasto aproximado del menú");
    asegurarEspacio(31);
    doc.setFillColor(...verdeClaro);
    doc.roundedRect(margen, y, anchoUtil, 26, 3, 3, "F");
    datoEtiqueta("Gasto total aproximado", formatearMoneda(gastoTotal), margen + 5, y + 8, 70);
    datoEtiqueta("Promedio por alimento", formatearMoneda(gastoPromedio), margen + 75, y + 8, 55);
    datoEtiqueta("Porciones registradas", String(menuUsuario.length), margen + 140, y + 8, 35);
    datoEtiqueta("Aviso", "Los precios son estimaciones tomadas de la base de datos y pueden variar según tienda, marca, ciudad o temporada.", margen + 5, y + 19, 170);
    y += 33;
    ...
    resumenCategorias.forEach(function(fila, indice){ // Recorre cada elemento de la lista y ejecuta la función para cada uno.
        ...
        doc.text(fila.kcal + " kcal", margen + 133, y);
        doc.text(formatearMoneda(fila.gasto), margen + 158, y); // Escribe este texto dentro del documento PDF.
    ...
    const columnas = { // Posiciones de las columnas de la tabla del PDF.
        numero: margen,
        alimento: margen + 8,
        porcion: margen + 78,
        kcal: margen + 116,
        gasto: margen + 131,
        categoria: margen + 153
    };
    ...
    function encabezadoTablaAlimentos(){ // Inicia la función encabezadoTablaAlimentos para realizar esta tarea cuando sea llamada.
        ...
        doc.text("Gasto", columnas.gasto, y + 5.3);
    ...
    menuUsuario.forEach(function(item, indice){ // Recorre cada elemento de la lista y ejecuta la función para cada uno.
        const nombreLineas = doc.splitTextToSize(String(item.alimento), 65); // Crea una variable y guarda en ella el valor calculado en esta línea.
        const categoriaLineas = doc.splitTextToSize(String(item.categoria), 27); // Crea una variable y guarda en ella el valor calculado en esta línea.
        ...
        doc.text("$" + calcularGastoAlimento(item).toFixed(2), columnas.gasto, y);
    ...
    textoParrafo("• Total de registros: " + menuUsuario.length + " alimentos. Alimentos diferentes: " + alimentosUnicos + ".");
    textoParrafo("• Gasto total aproximado del menú: " + formatearMoneda(gastoTotal) + ". Promedio por alimento: " + formatearMoneda(gastoPromedio) + "."); // Ejecuta esta instrucción como parte de la lógica de la función actual.
    ...
    textoParrafo("Este reporte es informativo y fue generado a partir de los alimentos capturados por el usuario en NutriUIN. Las calorías y los gastos dependen de la base de datos, de las porciones seleccionadas y de precios aproximados que pueden variar."); // Ejecuta esta instrucción como parte de la lógica de la función actual.
    ...
    doc.setProperties({ // Guarda metadatos descriptivos dentro del archivo PDF.
        ...
        subject: "Alimentos, calorías, gasto aproximado y resumen del menú del usuario", // Define este dato descriptivo en las propiedades del PDF.
